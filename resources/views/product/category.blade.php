    ...
    <div class="row">
        <div class="col-12 col-md-9">
                @component("component.card")
                    ...
                    @slot('title')  {{ isset($productCategory) ? 'Edit Product Category ' : "Add Product Category" }}@endslot
                    @slot('button')  @endslot
                    @slot('body')
                        @if(isset($productCategory))
                        <div>
                            <form action="{{ route('product_category.update',$productCategory->id )}}" method="post" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <div class="form-row align-items-end">
                                    <div class="col-12 col-md-8">
                                        <label for="name">Title</label>
                                        <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="name" value="{{ $productCategory->name }}" placeholder="Title">
                                        @error('name')
                                    ...
                                    @foreach(\App\ProductCategory::latest()->get() as $c)
                                        <tr>
                                            ...
                                            <td class="control-group d-flex" style="vertical-align: middle; text-align: center">
                                                <a href="{{ route('product_category.edit',$c->id) }}" class="btn mr-2 btn-outline-warning btn-sm">
